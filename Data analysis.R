...
ggplot(se, aes(thi, mean, colour = thi)) +
  ...
  geom_errorbar(aes(ymin = mean-se, ymax = mean+se), width = 0.2, size = 1.5, show.legend = FALSE) +
  ylab("THI") + xlab("Time") + ylim(0,45) + theme_classic(base_size = 17) +
...
df_imp %>% 
  ...
  group_by(case_id) %>% 
  fill(group_id, questionnaire, age, sex, duration_tin_mon, thi_pre, phq9_score, bfi2_extraversion, bfi2_neg_emotion, .direction = c("downup")) -> df_imp
...
# Figure 4: visualisation of EMA distress & loudness
...
# 6. Compare THI with EMA data (Figure 6) ----
